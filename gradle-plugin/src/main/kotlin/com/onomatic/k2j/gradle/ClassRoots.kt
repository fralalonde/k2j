 ...
 *
 * `ConversionRequest` (frozen core API) takes a single `classesRoot`, so when more than one
 * candidate carries Kotlin classes the metadata-bearing roots are copied into one staging directory
 * (relative package layout preserved) and that directory is surveyed. The core change worth making
 * is for `ConversionRequest.classesRoot` to become `classesRoots: List<Path>` so no staging copy is
 * needed; see gradle-plugin/IMPLEMENTATION.md.
 */
