...
class K2jEndToEndTest {
    ...
    fun `end-to-end convert on corpus converts every unit and refuses typealias sources`() {
        ...
        // Memberless interfaces/facades are conversion targets now (defect B): IEntityId, Holder,
        // TypeAliasesKt and MixedKt all decompile to empty but valid Java types.
        assertEquals(
            listOf(
            ...
            "every corpus class must convert"
        )

        val deletableNames = manifest.deletableSources.map { Path.of(it).fileName.name }.sorted()
        assertEquals(listOf("Bridge.kt", "CancelActivityLike.kt", "Outer.kt", "Single.kt"), deletableNames)
    ...
    fun `delete-sources on a copy deletes only fully represented sources`() {
        ...
        assertEquals(listOf("Bridge.kt", "CancelActivityLike.kt", "Outer.kt", "Single.kt"), deletedNames)
