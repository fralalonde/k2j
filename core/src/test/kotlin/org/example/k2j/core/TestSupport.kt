...
object TestSupport {
    ...
    /**
     * A jar from this machine's Gradle module cache, found by file name (`kotlin-stdlib-2.4.0.jar`),
     * or null when the cache does not hold it. Compiling generated Java needs exactly the two
     * libraries the decompiled text imports — kotlin-stdlib and the JetBrains annotations — and
     * resolving them from the cache keeps the test free of a hand-written stub that could drift from
     * the real thing. Callers skip (loudly) when the jar is missing.
     */
    fun cachedJar(fileName: String): Path? {
        val root = Path.of(System.getProperty("user.home"), ".gradle/caches/modules-2/files-2.1")
        if (!Files.isDirectory(root)) return null
        return try {
            Files.walk(root, 6).use { stream ->
                stream.filter { Files.isRegularFile(it) && it.fileName.toString() == fileName }
                    .findFirst()
                    .orElse(null)
            }
        } catch (t: Throwable) {
